			...
			for(int col=0; col<this.COLUMNS; col++) {
				...
				for (int i=0; i<count.length; i++) {
					if (count[i] == 1 && this.candidateList(row, col).contains(String.valueOf(i+1))) {
						...
						this.updateCLists();
						//System.out.println("REPLACING WITH " + val);
						//System.out.print("at pos " + row + " " + col + ": ");
						//for (int y=0; y<count.length; y++) 
						///	System.out.print(y + " " + count[y] + "; ");
						//System.out.println();
						return;
					}
				}
				...
				for(int col=0; col<this.COLUMNS; col++) 
				{
					...
					for(int col2=0; col2<this.COLUMNS; col2++) 
					{
						...
						if (cList2.size() == 2 && c2.getCoordinate() != c.getCoordinate() && cList2 != cList) {
							//System.out.println("naked pairs at " + row + " " + col + " " + col2);
